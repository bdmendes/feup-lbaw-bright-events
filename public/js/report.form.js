  ...
  fetch('/api/reports/form')
      ...
      .then((modalText) => {
        ...
        select.addEventListener(
            'input', () => {select.classList.remove('is-invalid')});

        let text = document.getElementById('report-form-description');
        text.addEventListener(
            'focus', () => {text.classList.remove('is-invalid')});
...
function submitReport() {
  ...
  if (document.getElementById('report-form-description').value === '' ||
      document.getElementById('report-form-description').value.length > 1000) {
    document.getElementById('report-form-description')
  ...
  fetch(form.action, {method: 'POST', body: new FormData(form)})
      .then((response) => {
        if (response.ok) {
          bootstrap.Modal.getInstance(document.getElementById('reportModal'))
              .hide();
        }
      })
}
